...
// Copia de la funcion get_ray de la camara, de manera que se pueda testear
// utilizando un random vector como parametro
...
TEST(CameraTest, Getting_Ray) {
  ...
  camera camaraPrueba =
      camera(lookfrom, lookat, vup, 20, aspect_ratio, aperture, dist_to_focus);

  vec3 randomVec = random_in_unit_disk();

  vec3 offsetEsperado =
      (camaraPrueba.u * randomVec.x() + camaraPrueba.v * randomVec.y()) *
      aperture / 2;
  vec3 origenEsperado = lookfrom + offsetEsperado;
  vec3 direccionEsperada = camaraPrueba.lower_left_corner +
                           camaraPrueba.horizontal + 2 * camaraPrueba.vertical -
                           lookfrom - offsetEsperado;
  ray rayoEsperado = ray(origenEsperado, direccionEsperada);
  EXPECT_TRUE(
      compare_rays(get_ray(camaraPrueba, 1, 2, randomVec), rayoEsperado));
}