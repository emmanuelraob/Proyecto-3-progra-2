...
hit_record hit_record_object_1 = {vec3_object_1, vec3_object_2, ptr_to_material_1, 0.5, true};
...
class mock_material : public material {
 public:
      MOCK_METHOD(bool, scatter, (const ray& r_in, const hit_record& rec, color& attenuation, ray& scattered), (const, override));
};
...
TEST(MaterialTest_Test, MaterialConstructorTest){
	//mock_material* mock_material_obj = dynamic_cast<mock_material*>(mock_material_obj);
}
...
TEST(LambertianTest, LambertianScatterTest_TrueIf_1_Argument){
	...
	vec3& vec3_reference_Lambertian_TrueIf_Argument = vec3_object_Lambertian_TrueIf_Argument;
	
	ray& r_in = ray_object1;
	hit_record rec_obj = {vec3_object_1, vec3_object_Lambertian_TrueIf_Argument, ptr_to_material_1, 0.5, true};
	...
	bool scatter_result = lambertian_obj.scatter(r_in, rec, attenuation, scattered);
...
TEST(LambertianTest, LambertianScatterTest_FalseIf_1_Argument){
	...
	vec3& vec3_reference_Lambertian_FalseIf_Argument = vec3_object_Lambertian_FalseIf_Argument;
	
	ray& r_in = ray_object1;
	hit_record rec_obj = {vec3_object_1, vec3_object_Lambertian_FalseIf_Argument, ptr_to_material_1, 0.5, true};
	hit_record& rec = rec_obj;
	vec3& attenuation = vec3_object_1;
	ray& scattered = ray_object1;
...
TEST(MetalTest, Metal_ScatterTest_1_Argument){
	ray& r_in = ray_object1;
	hit_record rec_obj = {vec3_object_1, vec3_object_2, ptr_to_material_1, 0.5, true};
	hit_record& rec = rec_obj;
	vec3& attenuation = vec3_object_1;
	ray& scattered = ray_object1;
...
TEST(MetalTest, Metal_ScatterTest_4_Argument){
	...
	scattered_ref_to_expect.orig = rec.p;
	scattered_ref_to_expect.dir = reflected + metal_obj.fuzz*random_in_unit_sphere();
...
TEST(DielectricTest, DielectricScatterTest_TrueIf_1_Argument){
	ray& r_in = ray_object1;
	hit_record rec_obj = {vec3_object_1, vec3_object_2, ptr_to_material_1, 0.5, false};
	hit_record& rec = rec_obj;
	vec3& attenuation = vec3_object_1;
	ray& scattered = ray_object1;
	...
	bool scatter_result = dielectric_obj.scatter(r_in, rec, attenuation, scattered);
	EXPECT_TRUE(scatter_result);
...
TEST(DielectricTest, DielectricReflectanceTest){
	...
	double reflectance_result_to_expect = step_1_reflectance + (1-step_1_reflectance)*pow((1 - 0.4),5);
